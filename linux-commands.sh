...
# assign exedcute permission to your script file, ensure it is stored in a directory within $PATH and has #!/bin/sh at top:
hello.sh
...
# [!0-9]  !=negate, match anything except 0 to 9
...
sort /student_files/uniqFile | uniq -d
...
# s => scan   -L => list
# find ip address of the machine that has nixos installed
sudo nmap -sL 10.0.1.0/24 | grep nixos

sudo nmap -sn 10.0.1.147/32  # works same as ping

sudo nmap 10.0.1.147  # port scan on all ports of this device, guesses what process is using that port

sudo nmap -p2323 10.0.1.147  # scan an specific port
...
# User Interfaces => GUI (graphical user interface), SHELL (command line interface)
# types of shell: Bourne shell / POSIX shell (sh), Cshell (csh), Korn shell (ksh), Bourne again shell(Bash), Z shell (Zsh)
# bash in the Linux default

# User Process => initiated by a regular user account and runs in 'user space'
# Daemons => any process designed to run in the background by an specific event or condition
# usually an ongoing service managed as such by the root user, often start at boot time and run until shutdown
# kernel process => like daemons, except kernel processes have full access to kernel data structure
# and execute only in 'kernel space'.

# Environment Variable => an env variable is a variable that is available to any child process of the shell
...
# your home directory holds files and subdirectories you create
...
/etc
# /etc/profile -> .bash_profile   home directory
#                 .bash_login -> .bashrc   creates aliases, command history and prompt
#                 .profile   completes the PATH env variable
...
# remove execution access from group fish
setfacl -x g:fish myfile

# shows all the permissions in details, including access from specific users
getfacl myfile
...
# change the group of the file .bash_history
...
# -print0 => This uses null characters as delimiters, which safely handles all filenames.
...
# give info about logged-on users:
who
finger USERNAME
users
...
# umask controls the default permissions for newly created files and directories in Linux. It works by specifying which permission bits should be removed from the system defaults.

# When you create a file or directory, Linux starts with default permissions:
# files => 666 , rw-rw-rw-
# directories => 777 , rwxrwxrwx

# The umask value is then subtracted from these defaults to determine the actual permissions.

umask 022
# files become 644 rw-r--r--
# directories become 755 rwxr-xr-x

umask 002
# files become 664 (rw-rw-r--)
# directories become 775 (rwxrwxr-x)

umask 077
# files become 600 (rw-------)
# directories become 700 (rwx------)

# Check your current umask:
umask  # 0002 => default permissions

# To make it permanent, add the umask command to your ~/.bashrc or ~/.profile
...
# -u : gives back uid for the username
id -u root  # shows 0
...
ls -l /home/  # John's home directory still exists (not deleted by default)
...
# -c : comment  -g : group  -s : default shell
sudo useradd -c "User Comment" -g groupname -s /bin/bash username
sudo passwd username

# this will change user's Primary group
sudo usermod -g newgroup username

usermod -aG new_group username  # add user to a new group, this new group will become Seconday group of the user
usermod -aG sudo my_user  # add user to sudo group, which gives admin rights

# adds sudo capability to users in most RHEL-based systems
...
# see latest created groups
cat /etc/group
cat /etc/group | tail -n1
# Only readable by root (requires sudo/root privileges)
sudo tail -3 /etc/shadow
sudo tail -3 /etc/gshadow
# see latest created users
tail -3 /etc/passwd   # john:x:1001:1001:John Doe:/home/john:/bin/bash
...
su -p babak
# login as another user temporarily
su - Phil

# become root user without knowing root's password
sudo -i
# same but requires password
sudo su -  
...
# 0 = stdin (standard input), the standard input stream, accepts text as its input, by default from a keyboard
# 1 = stdout (standard output), sends text output from command, by default to screen
# 2 = stderr (standard error), sends error output from the command - also, by default to the screen
...
ls -l linefile fakefile > goodfile 2> badlife

# linefile exists and fakefile does not
# > goodfile  :  stdout is written to the file goodfile
# 2> badfile  :  stderr is written to the file badfile

# input redirection
wc -l /etc/passwd    # 37 /etc/passwd
...
# list all running processes along with detailed information about each
# a — show processes for all users (not just yours)
# u — display the process's user/owner
# x — include processes that are not attached to a terminal
ps aux
...
# if lsof doesnt show anything, it is probably due to lack of sudo permissions

# The +d option tells lsof to list all open files in the specified directory (but not in its subdirectories)
lsof +d .  

# lists all open files in /var/log and its subdirectories
lsof +D /var/log  
...
# which : is used to locate the executable file associated with a given command by search PATH environment varialbe
# PATH : contains an ordered list of all directories that Unix will search for executables when running a command
which -a touch
# /usr/bin/touch
# /bin/touch
